...
# In der hier gezeigten Form, wird die einfachste Möglichkeit mittels x-Koordinatenvergleich umgesetzt.
nr_of_points_left = 0
nr_of_points_right = 0
for p in points:
    if p[0] < centerpoint[0]:
        nr_of_points_left += 1
    else:
        nr_of_points_right += 1

print(nr_of_points_left, nr_of_points_right)
...
solution = contours_internal.copy()
contours_internal = cv2.putText(contours_internal, 'innere Konturen', (25, 25), font, 0.8, RED, 2, cv2.LINE_AA)
solution = cv2.putText(solution, 'Loesung', (25, 25), font, 0.8, RED, 2, cv2.LINE_AA)
solution = cv2.putText(solution, f'{nr_of_points_left:d}', (150, 90), font, 2.0, RED, 8, cv2.LINE_AA)
solution = cv2.putText(solution, f'{nr_of_points_right:d}', (430, 90), font, 2.0, RED, 8, cv2.LINE_AA)

result_1 = np.hstack([image, contours_all])
result_2 = np.hstack([contours_external, contours_internal])
result_3 = np.hstack([solution, solution*0])
result_4 = np.hstack([gray, blurred])
result_5 = np.hstack([threshold, threshold_adaptive])
result_6 = np.hstack([eroded, dilated])

result_stack_color = np.vstack([result_1, result_2, result_3])
result_stack_mono = np.vstack([result_4, result_5, result_6])

# schreibe das Bild in eine Datei
cv2.imwrite('img/solution.png', np.hstack([image, solution])[0:-2, 0:-2])
